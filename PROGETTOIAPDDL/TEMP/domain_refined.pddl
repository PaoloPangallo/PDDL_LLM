...
(define (domain tower-of-varnak)
  ...
  :predicates
    ...
    (sleeping ?m)
    (awake ?m)
    (inventory-item ?a ?o)  ;; Added inventory-item predicate for the goal
  :actions
    (move
  ...
  :init
    (and
      ...
      (sleeping ice-dragon)
      (inventory-item hero sword-of-fire)) ;; Added inventory-item initialization for the hero and sword
  :types
    (location village tower-of-varnak)
    (monster sleeping awake)
    (object inventory-item sword sword-of-fire) ;; Defined object type as inventory-item and gave it a specific instance "sword-of-fire"
  :functions
    ...
    (sleeping-awake ?m 0)
  :action-precedence (:action wake-dragon :action pickup :action move) ;; Added action precedence to ensure waking the dragon before picking up or moving
  (:action wake-dragon
    :parameters (?m)
