(define (domain logistics)
  ...
  (:types agent location object vehicle)
  (:predicates
    (at ?a - agent ?l - location) ; Resolved error: ?a parameter added
    (at ?r - vehicle ?l - location)
    (at ?o - object ?l - location)
    (has ?a - agent ?o - object)
    (in ?a - agent ?r - vehicle)
  )
  (:action mount-rover
    :parameters (?a - agent ?r - vehicle ?l - location) ; Resolved error: ?a parameter added
