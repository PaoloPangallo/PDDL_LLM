...
def load_lore(lore_filename: str, folder: str = "lore") -> Dict[str, Any]:
    """
    Carica un file JSON contenente la lore.
    :param lore_filename: nome del file, es. 'example_lore.json'
    :param folder: cartella dove cercare la lore (default: 'lore')
    :return: dizionario Python con la lore
    """
    path = os.path.join(folder, lore_filename)
    if not os.path.exists(path):
        raise FileNotFoundError(f"File lore non trovato: {path}")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)

def save_lore(lore_filename: str, lore: Dict[str, Any], folder: str = "uploads") -> None:
    """
    Salva un dizionario come file JSON.
    :param lore_filename: nome del file di output, es. 'lore_updated.json'
    :param lore: dict da serializzare
    :param folder: cartella di destinazione (default: 'uploads')
    """
    os.makedirs(folder, exist_ok=True)
    path = os.path.join(folder, lore_filename)
    with open(path, "w", encoding="utf-8") as f:
